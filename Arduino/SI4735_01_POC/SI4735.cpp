...
SI4735::SI4735()
{
    lastMode = -1; //!<  Stores the last mode used.

    currentAvcAmMaxGain = 48;          //!<  Stores the current Automatic Volume Control Gain for AM. Default value is 48.
    currentClockType = XOSCEN_CRYSTAL; //!< Stores the current clock type used (Crystal or REF CLOCK)
    currentInterruptEnable = 0;        //!< If you are using interrupt, this variable stores 1.

    refClock = 31768;     //!< Frequency of Reference Clock in Hz.
    refClockPrescale = 1; //!< Prescaler for Reference Clock (divider).
    refClockSourcePin = 0; //!< 0 = RCLK pin is clock source; 1 = DCLK pin is 
        // 1 = LSB and 2 = USB; 0 = AM, FM or WB
    deviceAddress = SI473X_ADDR_SEN_LOW; //!<  Stores the current I2C bus address.

    // Delays
    maxDelaySetFrequency = MAX_DELAY_AFTER_SET_FREQUENCY; //!< Stores the maximum delay after set frequency command (in ms).
    maxDelayAfterPouwerUp = MAX_DELAY_AFTER_POWERUP;      //!< Stores the maximum delay you have to setup after a power up command (in ms).
    maxSeekTime = MAX_SEEK_TIME;                     //!< Stores the maximum time (ms) for a seeking process. Defines the maximum seeking time.
    
    currentInterruptEnable = 0;        //!< If you are using interrupt, this variable stores 1.
    volume = 32; //!< Stores the current vlume setup (0-63).

    currentAudioMode = SI473X_ANALOG_AUDIO; //!< Current audio mode used (ANALOG or DIGITAL or both)
    audioMuteMcuPin = -1;
    currentSsbStatus = 0;
}
