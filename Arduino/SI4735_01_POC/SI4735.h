...
protected:
    ...
    uint16_t maxDelayAfterPouwerUp;      //!< Stores the maximum delay you have to setup after a power up command (in ms).
    ...
    uint8_t currentAvcAmMaxGain;          //!<  Stores the current Automatic Volume Control Gain for AM. Default value is 48.
    uint8_t currentClockType; //!< Stores the current clock type used (Crystal or REF CLOCK)
    uint8_t currentInterruptEnable;        //!< If you are using interrupt, this variable stores 1.
